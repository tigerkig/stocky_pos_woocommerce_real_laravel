...
class ProductsController extends BaseController
{
    ...
    public function index(request $request)
    {
        ...
        foreach ($products as $product) {
            ...
            $item['parent_code'] = $product->parent_code ? $product->parent_code : $product->code;
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            \DB::transaction(function () use ($request, $temp_pos_id, $temp_pos_var_id, $temp_pos_name, $temp_pos_price) {
                ...
                $Product->parent_code = $request['is_child'] == 'true' ? $request['parent_code'] : '';
                ...
                $Product->is_child = $request['is_child'] == 'true' ? 1 : 0;
                ...
                if ($warehouses) {
                    ...
                    if($request['parent_code']) {
                        if (Product::where('code', '=', $request['parent_code'])->exists()) {
                            $item = Product::where('code', '=', $request['parent_code'])->get();

                            $update_quantity = product_warehouse::where('product_id', $item[0]->id)
                                ->where('deleted_at', '=', null)
                                ->where('deleted_at', '=', null)
                                ->get();

                            foreach ($update_quantity as $key => $value) {
    ...
    public function Get_Products_Details(Request $request, $id)
    {
        ...
        $item['id'] = $Product->id;
        $item['code'] = $Product->code;
        $item['parent_code'] = $Product->parent_code;
        ...
        $item['is_child'] = $Product->is_child == '1' ? true : false;
